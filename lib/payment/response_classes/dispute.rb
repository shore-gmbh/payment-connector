module ShorePayment
  ...
  class Evidence < StripeHash
    SUPPORTED_ATTRIBUTES = %i(
      ...
      receipt customer_signature customer_communication uncategorized_file
      uncategorized_text service_date service_documentation shipping_address
  ...
  class Dispute < StripeHash
    SUPPORTED_ATTRIBUTES = %i(
      id status reason amount_cents currency created_at organization_id due_by
      has_evidence past_due submission_count evidence
    ).freeze
